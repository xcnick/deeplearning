...
class TestBertModel:
    ...
    def test_model(self):
        ...
        assert pooled_output.shape == [
            self.batch_size, self.config.hidden_size
        ]
        assert prediction_scores.shape == [
    ...
    def test_tf_and_huggingface_compare(self):
        ...
        for tf_param, hf_param in zip(self.model_tf.state_dict(),
            ...
            assert paddle.equal_all(self.model_tf.state_dict()[tf_param],
                                    self.model_hf.state_dict()[hf_param])

        for tf_param, pt_param in zip(self.model_hf.state_dict(),
                                      self.model.state_dict()):
            assert paddle.equal_all(self.model_hf.state_dict()[tf_param],
    ...
    def test_model_forward(self):
        ...
        assert np.max(
            np.abs(hf_encoder_output.numpy() - last_hidden_state)) < 1e-3
        ...
        assert np.max(
            np.abs(tf_encoder_output.numpy() - last_hidden_state)) < 1e-3
        ...
        assert np.max(
            np.abs(encoder_output.numpy() - last_hidden_state)) < 1e-3
