...
class TFEncoderLayer(tf.keras.layers.Layer):
    def __init__(
        ...
        **kwargs,
        ...
        self.self = TFMultiHeadAttention(
...
class TFMultiHeadAttention(tf.keras.layers.Layer):
    ...
    def call(
        ...
        # multi head
        query = tf.reshape(query, (batch_size, -1, self.num_attention_heads, self.dims_per_head))
        query = tf.transpose(query, perm=[0, 2, 1, 3])
        key = tf.reshape(key, (batch_size, -1, self.num_attention_heads, self.dims_per_head))
        key = tf.transpose(key, perm=[0, 2, 1, 3])
        value = tf.reshape(value, (batch_size, -1, self.num_attention_heads, self.dims_per_head))
