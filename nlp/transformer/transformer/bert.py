...
from .transformer import AddNormLayer, Encoder
...
def load_tf_weights_in_bert(model, config, tf_checkpoint_path, with_mlm=True):
    try:
...
def load_huggingface_weights_in_bert(model, config, pytorch_model_path, with_mlm=True):
    try:
