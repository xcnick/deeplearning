...
class PolyLRScheduler(tf.keras.callbacks.Callback):
    ...
    Args:
        ...
        power: float,
            new_lr = base_lr * (1 - global_step / total_steps)**power.
        min_lr: float, minimum learning rate.
        optimizer: str, the name of optimizer that using this
            learning rate scheduler, default to apply to all optimizers.
    """
    ...
    def __init__(
        ...
        if self.warmup_iters > 0 and self.warmup_lr == self.base_lr:
            ...
            print_log(
                f"`PolyLRScheduler` got warmup learning rate is equal to "
                f"base learning rate ({print_lr_}), "
                f"there is no need for warmup,"
                f"set warmup iterations {self.warmup_iters} -> 0",
                self.logger,
    ...
    def on_train_batch_begin(self, batch, logs=None):
        if 0 < self.global_step <= self.warmup_iters:
            lr = (
                self.warmup_lr + (self.base_lr - self.warmup_lr) /
                self.warmup_iters * self.global_step)
        else:
            lr = self.base_lr * (
                1 - self.global_step / self.total_steps)**self.power
