...
class Executor:
    ...
    def run(self, feed_dict):
        ...
        for node in topo_order:
            ...
            compute_val = node.op.compute(node, input_vals)
            if not isinstance(compute_val, np.ndarray):
                compute_val = np.array(compute_val)
            node_to_val_map[node] = compute_val
            # default_val = node_to_val_map.get(node, np.zeros(compute_val.shape))
            # node_to_val_map[node] = np.array(compute_val + default_val)
        # Collect node values.
...
def gradients(output_node, node_list):
    ...
    for node in reverse_topo_order:
        ...
        for i in range(len(node.inputs)):
            node_to_output_grads_list[node.inputs[i]] = node_to_output_grads_list.get(
                node.inputs[i], []
            )
